...
public sealed class MacroPlan : INotifyPropertyChanged, IRunCondition
{
    ...
    // 仅 UI：本方案是否为当前的定时启动目标（全局单一，显示时区分标记）
    private bool _isScheduled;
    [JsonIgnore] public bool IsScheduled { get => _isScheduled; set { if (_isScheduled != value) { _isScheduled = value; Raise(nameof(IsScheduled)); } } }
