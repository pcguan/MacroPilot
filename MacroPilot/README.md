...
- 标注（`Annot`）的命中判定**只认轮廓、不认内部**（`Services/HitGeometry`，纯几何、有单测）：点边框才选中/拖动，内部永远留给"接着画新图形"——这样嵌套画图零歧义，也就不必再按"手上有没有握着工具"分情况。选中后手柄优先于轮廓；轮廓上按下要用 `RectPicker.BeginWith(p,"move")`，直接 `Begin` 会被它自己判成"拉边"。撤销是**回滚闭包栈**（新增/改动/删除各压一条），不是「删掉最后一个」。马赛克预览＝裁剪→按块尺寸缩小→`NearestNeighbor` 放大，块尺寸与 GDI 烧录共用 `MosaicBlock()`，否则所见非所得。
