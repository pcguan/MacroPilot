...
- **抓屏两条路径**（`ScreenCapture`）：DXGI 桌面复制（物理机实测全屏 5.7ms） + GDI `CopyFromScreen` 回退（40ms）。**启动时 `Probe` 一次定乾坤**（RDP/虚拟机/老驱动建不出复制会话→整场 GDI，corp-win 的 RDP 会话实测就是这情况）；探测通过后单次失败（AccessLost/安全桌面/跨屏区域/旋转屏）仍就地回退 GDI。运行日志的开始行会标明本场模式。
