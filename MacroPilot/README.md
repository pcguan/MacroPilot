...
- 标注（`Annot`）的命中判定是**两段式**（几何部分在 `Services/HitGeometry`，纯函数、有单测）：未选中时**只认轮廓**（点边框＝选中，内部不响应，留给"接着画新图形"）；选中后该图形**独占整块范围**，内部＝move、边/角＝缩放（直接复用 `RectPicker.HitTest`），点它以外＝退出选中。**有选中就不画新图形**——这条硬规则让"嵌套画"与"拖走已有图形"永远不会撞车。
- 选中状态在"用户显然改去做别的事"时要主动解除：**切换工具即解除**（否则手柄还挂着，下一笔又被「有选中就不画」挡住）。改颜色/粗细不解除——那是在操作选中项本身。
- 可选中的只有**矩形 / 椭圆 / 箭头**；画笔、马赛克、文字画完即定稿（要改就撤销重画）——笔迹类参与选中只会在连续涂抹时误抓到自己刚画的那一笔。
- 马赛克是**笔迹类**（与画笔同族，几何存在 `Pen` 点列里）：预览＝整块像素化的图 + 用笔迹加宽出来的形状做 `Clip`；烧录＝同一套块网格算均色，但只写笔刷掩码覆盖到的像素，两边因此完全一致。
- 标注一律夹在**截图选区**内（画、拖、改大小、方向键都夹）：移动用 `ClampOffset`（整体平移），改大小用 `ClampRect`（逐边夹）——移动时逐边夹会把图形压扁。
- 颜色 / 粗细存在 `Annot` 自身（不是全局常量），预览与 GDI 烧录都按各自的值画；箭头是**尾细头宽的实心多边形**（`ArrowPolygon`，两处共用同一份几何），文字用粗细档位换算字号。选中反馈只用**贴着轮廓的白色手柄**——别给图形描白边或外扩虚线框，看起来会像图形变大了、边缘发脏。
- **删除标注不能走 `SelectAnnot`**：它会给"上一个选中项"补画一次高亮，而那个对象刚被删掉——视觉元素被加回画布、又已不在 `annots` 里，撤销栈再也够不着它（表现为「撤销到底还剩几个图形赖着不走」）。统一走 `Detach()`：先摘选中、再移除视觉元素并清空 `Visuals`。撤销是**回滚闭包栈**（新增/改动/删除各压一条），不是「删掉最后一个」。马赛克预览＝裁剪→按块尺寸缩小→`NearestNeighbor` 放大，块尺寸与 GDI 烧录共用 `MosaicBlock()`，否则所见非所得。
...
> 同步到构建机一律用 `tools/sync_corpwin.sh`（全绝对路径 + 传完自检）。**别在命令行里临时 `cd .. && tar ...`**——
> cwd 差一级就会把树传错位置，而 `dotnet build` 照样报"成功"（编的是上次的旧代码），于是"构建通过"验证的根本不是本次改动。这个坑踩过两次。
