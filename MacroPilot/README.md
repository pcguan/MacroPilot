...
| `Services/` | `Storage` 持久化、`MacroRunner` 执行引擎、`UpdateService` 在线更新、`Changelog`（内置更新日志）、`ScreenMatch` 图片搜索匹配、`ScreenCapture`（DXGI 桌面复制 + GDI 回退双路抓屏）、`ImageStore` 图片外置+孤儿清理、`InputRecorder` + `RecordingCompiler`（宏录制：钩子采集 + 纯函数归纳）、`HitGeometry`（覆盖层命中几何，纯函数）、`RandomText`（逆向正则：按模式生成随机串）、`WindowMemory`（窗口几何记忆）、`WindowActivator`、`MouseTraceRecorder` 轨迹录制、`ThemeManager`、`PreciseTimer` |
| `App.xaml(.cs)` | 单实例（唤起已有窗口后静默退出，**不弹模态**）+ 按需提权 + 显式建窗口 + 全局统一 ToolTip 延迟 |
| `MainWindow.xaml(.cs)` | 主界面；partial 拆分：`.Dialogs`（编辑对话框，内含 `CoordBlock`/`RepeatBlock`/`TimeInputRow`/`EdgeCell`/`ClickImagePanel`/`RectPicker` 可复用块）、`.RunCondition`（运行条件编辑器 + 方案设置对话框）、`.Snip`（截图覆盖层：自动框窗 + 标注工具条）、`.Record`（宏录制入口与悬浮条）、`.Hud`（运行悬浮窗）、`.Tray`（系统托盘）、`.Schedule`（定时启动）、`.Update`（更新进度窗） |
...
- 跳转目标绑定 `MacroStep.Alias`（别名标签，v0.6.5 起）：解析纯按字符串找第一个同名者，**没有任何序号/身份同步逻辑**（`RefreshIndices` 只剩列表编号）。旧存档回退顺序：别名 → `JumpTargetId` → 序号。改别名要在编辑落地点同步方案内所有指向旧名的跳转；**粘贴副本要 `ClearAliases()`**（重名标签会让跳转指错），`RenewId()` 仍保留供旧格式 Id 解析不撞车。
...
- **重复检查**：不满足时按间隔轮询重判（走 `Wait`，暂停/停止照常响应），每次判定都记一条带相似度的日志。方案级历来就是「空转等到满足」，故忽略该勾选始终等待，但间隔与两个上限对它生效。
  - **0 是合法值，别再兜成默认**：间隔 0 = 判完立刻再判（此时仍要 `Gate` + 查取消，否则暂停/停止按不动）；次数上限与时长上限**同时生效、先到者结束**，各自 0 表示不限。
...
- **先粗后精（`Matcher` + `Decimate` + `Sat`）**：先把区域与模板缩小 k 倍粗筛，候选再回原图用**同一套判定**精验——命中与分数一字不差，省掉的只是不可能的位置。真实数据：0.91 秒 → 0.14 秒。
  **必须遍历 k² 个相位**：缩略图的块边界固定在 0,k,2k…，而目标在屏幕上的位置是任意的；只按一个相位缩图，没落在块边界上的目标就会被漏（实测 16 个相位丢 14 个）。按 (0..k-1, 0..k-1) 各缩一次就一定有一相严丝合缝。位置总数因此不变，省的是**每个位置的比较量（1/k²）**。区域的 k² 张缩略图靠积分图 `Sat` 一遍算好，否则光缩图就比搜索还贵。
  **何时启用**：位置数 ≥25 万、模板 ≥3000 像素、且缩完仍保留 ≥250 像素——三个门槛都是实测定的。模板太小/缩太狠时，准备工作（积分图 + k² 张缩略图，开销只跟区域面积有关）比省下的比较还多，实测会倒退到 0.4 倍。
- **并行度可配**（`ScreenMatch.MaxParallelism`，0=不限，由配置页 `MacroDocument.MatchMaxCores` 写入）：搜索会吃满选中的核，而本工具常与游戏同处一机。存"核数"且 0 表示全部——换机器后核数变了仍然正确，UI 再按 `Environment.ProcessorCount` 夹一次。实测（14 核、整屏 364×41）：全核 124ms / 7 核 135ms / 4 核 179ms / 1 核 600ms，**有效并行度只有 ~6**（积分图与缩略图那段是串行的），所以砍到一半核几乎不影响速度。
- **NMS 前的排序必须是全序**（分数降序 + 位置升序）：扫描是并行的、回收顺序不定，只按分数排的话同分候选进 NMS 的先后每次都可能不同 → 命中个数与顺序会飘，「点击第 N 个」不可复现。
- **抓屏两条路径**（`ScreenCapture`）：DXGI 桌面复制（物理机实测全屏 5.7ms） + GDI `CopyFromScreen` 回退（40ms）。**启动时 `Probe` 一次定乾坤**，且必须**遍历适配器选带输出的那个**——corp-win 实测有两个同名 Intel 适配器、其一没挂任何显示器，用"默认适配器"会恰好选中它而误判不支持（SSH/服务会话则是所有适配器都无输出，正确回退 GDI）；探测通过后单次失败（AccessLost/安全桌面/跨屏区域/旋转屏）仍就地回退 GDI。运行日志的开始行会标明本场模式。
- **一轮判定内共享抓屏**（`MacroRunner._roundShots`）：`Evaluate` 一轮里按（区域矩形）缓存抓屏，多条同区域图片条件只抓一次；轮与轮之间必须重抓（画面会变，这正是轮询的意义）。
- **模板两级缓存**：解码位图按【图片引用字符串】缓存（内容寻址无失效问题，运行条件与点击图片共用同一实例）；模板预处理（权重+排序，O(n log n)）按位图实例挂 `ConditionalWeakTable`——实例复用 → 预处理全程只算一次。
- **按行并行**（`Parallel.For`，共享缓冲只读）：位置数 = (区域宽−模板宽+1)×(区域高−模板高+1)，全屏 2560×1440 配 364×41 的模板就是 **307 万个位置**，每个位置平均还要比上千个像素才够爆早退预算——真实数据实测单线程 10 秒/次，并行后 0.9 秒（14 逻辑核），结果逐位一致。运行条件的"重复检查"会把这笔开销按次数翻倍，用户侧就表现为"两条日志之间隔了 33 秒"。
- 搜索开销与**区域面积**成正比，这是数量级最大的那个旋钮：区域从整屏缩到 800×300 还能再快 ~27 倍。UI 上要引导用户设限制区域。
- **搜索必须可取消**（`FindMatches(..., CancellationToken)`，逐行检查）：一次大区域搜索要几百毫秒到数秒，期间不理会取消的话，F11 得等这一整轮搜完才生效，手感就是"按了没反应"。判定链上的 `Evaluate`/`EvaluateOne`/`LocateImage` 都要把 ct 传下去。
- 改这个算法前先用真实截图离线验证：`FindIn(Bitmap, ...)` 就是为此把"抓屏"和"搜索"拆开的——单测（`ScreenMatchTests`）用合成位图跑确定性回归，压测则用 pc-guan 的真实截图 + 真实模板。
...
**次数 / 间隔（`RepeatBlock`）——两个字段，别混**
- **执行次数** `LoopCount`/`LoopDelayMs`：只重复**动作本体**（`RunOne`），条件判一次、监听走一遍。界面上按动作类型叫点击/滚动/按键/输入次数。
- **重复次数** `RepeatCount`/`RepeatDelayMs`：重复**整趟**（判条件 → 监听 → 本体），每趟都重新判条件、重新触发监听。引擎里由 `RepeatCycle` 包住叶子与组合两条路径；**中途产生跳转必须停止重复**，否则跳转被困在原地。
- 同一个 `RepeatBlock` 用 `forRepeat` 切换写哪一组字段，UI 完全一致。
...
**拖动窗口（`DragMode="Window"`）**
- 走**真实鼠标拖拽**而不是 `SetWindowPos` 直接搬：对方程序收到的是正常拖窗手势，贴边吸附、跨屏 DPI 切换等系统行为才会照常发生。
- 抓点取标题栏中部偏左（正中常被标签页占着，太靠右会压到关闭按钮）。位移＝目标左上角 − 当前左上角，而目标左上角由 `DragAnchor`（3×3 九宫格）反推：终点坐标对应的是窗口的哪个点由用户选，默认左上角。
- 编辑顺序必须是「选窗口 → 对齐点 → 终点坐标」，所以鼠标面板被拆成两段，中间夹着与「激活窗口」共用的那个窗口选择框。
- 窗口匹配复用 `WindowActivator.Find`（与「激活窗口」同一套 PID/进程/标题优先级），别再各写一份。

**嵌套对话框（`MakeDialog` / `_dlgStack`）**
- 编辑动作里还能再编辑监听动作，对话框是可以套开的。套开时三件事必须一起做：`Owner` 挂到**上一层**（z 序）、位置相对上一层**错开**（否则同名对话框正好完全重叠，看着像上一层消失了）、几何记忆的 key **按层加后缀**（共用一份的话，内层关闭时会把错开后的位置写回去，下次外层从那儿开再错一次，每套一层漂 36px）。
- 上一层是谁靠 `_dlgStack` 认（对话框都是模态，栈顶必然是父）；栈里要兜底清掉"建了却没 Show"的残留。
...
**动作剪贴板（`_clip` / `ClipChanged`）**
- 动作列表与监听挂点**共用一个剪贴板**，所以列表里的动作能贴成监听动作、反之亦然（组合除外）。
- 改剪贴板一律走 `SetClip`：粘贴按钮的可用状态是**建按钮时算的**，直接赋值 `_clip` 的话，先建好的那些按钮永远是灰的（表现为"复制了却粘不到别的挂点"）。订阅方记得在窗口 `Closed` 时退订。

**下拉框与滚轮（`App.OnStartup` 的类处理器）**
- 收起状态下滚轮**不改选项**（滚页面顺手把「左键」滚成「中键」且毫无察觉，是最坑的一类误操作），展开状态照常滚列表。
- **光置 `Handled` 不够**：事件同时也到不了外层 `ScrollViewer`，表现为"鼠标停在下拉框上时窗口滚不动"。吞掉之后必须把这一下**转交给父元素**（`RaiseEvent` 冒泡版 `MouseWheelEvent`）继续冒泡。

**宏录制（`InputRecorder` + `RecordingCompiler` + `.Record`）**
- 三层分工：钩子采集（只入队原始事件，回调轻到不能再轻——LowLevelHooksTimeout 那套既有结论）→ **纯函数归纳器**（事件流 → 动作列表，坐标换算注入，规则全部单测）→ UI 壳。改归纳规则只动 `RecordingCompiler`，跑测试即可验证。
- 悬浮条**固定不可拖**：采集层按它的矩形排除"点完成这一下"，位置必须整场稳定。`WS_EX_NOACTIVATE` 不抢焦点——录的就是用户在目标程序里的操作。
- 拖动判定看**轨迹最大偏移**而不是起终点距离（画圈回原地也是拖动）；长按的系统自动重复 KeyDown 采集层与归纳器各去重一次。
- 中文 IME 打字录到的是拼音键序列，无解——提示用户改文本动作。
