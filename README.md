...
- 动作类型分层：**输入 → 鼠标**（点击 / 点击坐标 / 点击图片 / 移动 / 移动图片 / 拖动 / 滚轮）、**输入 → 键盘**（含修饰键组合捕获）；**运行 → 等待 / 激活窗口 / 跳转**；**组合**
  - 点击＝纯点击（当前位置）；点击坐标＝先移动到坐标再点击；拖动＝起点按下→移到终点→松开（起终点可跨屏）
  - **点击图片 / 移动图片**：在限制区域内搜索目标图，点击命中项，或只把光标移过去不点击（两者共用同一套图片设置）。目标图支持截图 / 本地导入 / 剪贴板粘贴（截图自动把限制区域设为截图位置）；限制区域可在屏幕上**拖动·缩放**调整并预览，调乱了可**一键还原到截图时的区域**；相似度 50–100 可调；命中多个时可指定点击**第几个**（从上到下、从左到右）
