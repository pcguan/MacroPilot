...
- **监听动作**：在动作生命周期的 7 个节点追加动作——**运行条件判断前 / 判断成功后 / 判断失败后 / 运行前 / 运行成功后 / 运行失败后 / 运行结束后**。与普通动作同款配置（可含循环、条件、组合），可递归嵌套；条件类三个节点仅在该动作设了运行条件时触发
...
- **条件不满足时重复检查**：按设定间隔反复判定直到满足（可设最大次数，0 = 不限），等待期间可正常暂停 / 停止；不勾选则条件不满足即跳过该动作
