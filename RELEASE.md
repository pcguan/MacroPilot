...
两个源都要验，且要**真下载下来核 SHA-256**，不能只看 API 返回说"已上传"：

```bash
# 自建源
curl -s https://nas.pcguan.cn/macropilot/version.json | head -3
curl -s https://nas.pcguan.cn/macropilot/MacroPilot-app.zip | sha256sum

# GitHub 源（记得带代理）
curl -sL -x http://192.168.33.9:7890 https://github.com/pcguan/MacroPilot/releases/latest/download/version.json | head -3
curl -sL -x http://192.168.33.9:7890 https://github.com/pcguan/MacroPilot/releases/latest/download/MacroPilot-app.zip | sha256sum
```

两边的 `version` 必须是新版本号，两边 zip 的 SHA-256 必须与 `version.json` 里的一致。
